// EVSimulate.jsx — the EV "Simulate" page (Mission Control), ported from the latest
// scenario engine. Ask anything → the deterministic EV engine builds a fault cascade,
// plays it on the live 3-D site (numbered stage bubbles + spark bursts + camera sweep),
// prices exposure with real ₹ formulas, runs a fault-specific Monte Carlo, ranks preset
// strategies AND searches the response action-space for the optimum, composes concurrent
// faults, switches across a network of sites, and hands off to "Fix It Live" — an
// interactive repair drill you play inside the twin. Fully client-side (no backend).
import React, { useEffect, useRef, useState } from 'react'
import EVWorld from './EVWorld.jsx'
import TakeCommand from './TakeCommand.jsx'
import { MODEL, assetById, faultsFor, resolveText, loadNetwork } from './ev/networkModel.js'
...
import { evOptimize, evMultifault } from './ev/decision.js'
import { RECORDS_KEY } from './ev/records.js'

const MARK = '◆ GoalCert Simulate'
const SUGGESTIONS = [
...
export default function EVSimulate() {
  ...
  // deterministic, instant, grounded — the copilot reads only the simulated facts.
  const groundedAnswer = (scn) => { setAnswer(fallback(scn.facts)) }
  ...
  const startLive = (assetId, faultId, q, answerFacts = null) => {
    ...
    groundedAnswer({ facts })
  }

  const onHorizon = (hk) => {
    setHorizon(hk)
    if (!selRef.current) return
    const { assetId, faultId } = selRef.current
  ...
  const submit = (text) => {
    ...
    const resolved = resolveText(q) || { assetId: 'TX-1', faultId: 'overload' }
    let s = 0
    const iv = setInterval(() => {
      s++; setReasonStep(s)
      if (s >= REASONING.length) { clearInterval(iv); startLive(resolved.assetId, resolved.faultId, q) }
    }, 430)
  }
  ...
  const toggleCond = (id) => {
    ...
    groundedAnswer(st.doNothing.scn)
  }
  ...
  const runMulti = () => {
    ...
    const mi = evMultifault(multiSel.map(m => ({ assetId: m.assetId, faultId: m.faultId })), conds)
    setMultiInfo(mi)
    let s = 0
    const iv = setInterval(() => {
      s++; setReasonStep(s)
      if (s >= REASONING.length) {
        clearInterval(iv)
        const cf = { site: MODEL.site, scenario: `${multiSel.length} concurrent faults`, faults: multiSel.map(m => m.label), combined_exposure_inr: mi.combined_exposure, if_separate_inr: mi.base_exposure, compounding_pct: mi.interaction_pct, breakdown: mi.parts }
  ...
  const pickStrategy = (st) => {
    ...
    groundedAnswer(st.scn)
  }

  // optimizer — client-side search over the response action-space
  useEffect(() => {
    const sel = selRef.current
    if (openTab === 'optimize' && sel && !optResult && !optBusy) {
      setOptBusy(true)
      Promise.resolve().then(() => setOptResult(evOptimize(sel.assetId, sel.faultId, conds))).finally(() => setOptBusy(false))
    }
    ...
    <div className="mc mc-live">
      ...
      <div className="mc-body">
        <main className="mc-center">
          <div className="mc-stage">
            <EVWorld live={{ ...live, __stages: stages }} focusId={focusId} height="clamp(400px, 58vh, 640px)" />
      ...
      {f && (
        <div className="report-print">
          <h1>GoalCert — EV Simulation Report</h1>
          ...
          <div className="rp-foot">Generated by GoalCert Simulate · the simulation engine is the source of truth.</div>
        </div>
      )}
